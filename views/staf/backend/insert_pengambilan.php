...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $barangTerpilih = $conn->real_escape_string($_POST['namaBarang']);
    $barangArray = explode(" - ", $barangTerpilih);
    $idBarang = isset($barangArray[0]) ? $barangArray[0] : ''; 
    $namaBarang = isset($barangArray[1]) ? $barangArray[1] : '';
    ...
    <p class="content">
    Melalui surat ini kami sampaikan bahwa kami, dari divisi ' . $namaStaf . ' bertujuan melakukan pengambilan barang ' . $idBarang . ' - ' . $namaBarang . ' sebanyak ' . $jumlah . ' yang dibutuhkan pada tanggal ' . $tanggalDiperlukan . ' untuk keperluan ' . $tujuan . '.
    </p>
    ...
    $sql = "INSERT INTO surat_pengambilan (no_surat, tanggal, tujuan, ID_barang, nama_barang, jumlah, status, link_surat, id_staf) 
            VALUES ('$noSurat', '$tanggalDiperlukan', '$tujuan', '$idBarang', '$namaBarang', '$jumlah', '$status', '$linkSurat', '$idStaf')";
