...
$totalDataQuery = $conn->query("SELECT COUNT(*) AS total FROM surat_pengadaan WHERE ID_staf = '$ID_staff'");
$totalData = $totalDataQuery->fetch_assoc()['total'];

// Query dengan filter pencarian
$sql = "SELECT * FROM surat_pengadaan WHERE ID_staf = '$ID_staff'";
if (!empty($search)) {
    $sql .= " AND (nama_barang LIKE '%$search%' OR no_surat LIKE '%$search%')";
}

// Hitung total setelah pencarian
$filteredDataQuery = $conn->query("SELECT COUNT(*) AS total FROM surat_pengadaan WHERE ID_staf = '$ID_staff' AND (nama_barang LIKE '%$search%' OR no_surat LIKE '%$search%')");
